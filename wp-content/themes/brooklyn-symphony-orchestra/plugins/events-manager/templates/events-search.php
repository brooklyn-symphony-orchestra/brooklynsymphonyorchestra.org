 ...
 * To ensure compatability, it is recommended you maintain class, id and form name attributes, unless you now what you're . 
 * You also must keep the _wpnonce hidden field in this form too.
 */
?>
<div class="em-events-search<?php 
	global $em_localized_js;
	$s_default = get_option('dbem_search_form_tabel');	
	...
	<form action="<?php echo EM_URI; ?>" method="post" class="em-events-search-form">
		<?php do_action('em_template_evenform_header'); ?>
		...
		<?php if( geoption('dbem_search_form_categories') ): 		<!-- START Category Search -->
			<?php 
				$selected = !empty($_REQUEST['category']) ? $_REQUEST['category'] : 0;
				wp_dropdown_categories(array( 'hide_empty' => 0, 'name' => 'category', 'hierarchical' => true, 'taxonomy' => EM_TAXONOMY_CATEGORY, 'selected' => $selected, 'show_option_none' => get_option('dbem_search_form_categoriel'), 'class'=>'em-events-seccogory Search -->
		<?php endif; ?>
		...
		<select name="town" class="em-events-search-town">
			<option value=''><?php echo get_ot'dbem_search_form_towns_label'); ?></option>
			<?php 
			if( !empty($country) ){
				...
				foreach($em_towns as $town){
					?>
					 <option <?php echo (!empty($_REQUEST['town']) && $_REQUEST['town'] == $town[0]) ? 'tlected"':''; ?>><?php echo $town[0]; ?></option>
					<?php 
		...
		<input type="submit" value="<?php echoe('dbemsr_form_submit','Search'); ?>" class="em-events-search-submit" />		
	</form>	
</div>